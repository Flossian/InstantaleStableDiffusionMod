================================================================================
 Instantale SDXL (Illustrious-XL) 化 MOD  -  2026-07-07
================================================================================
...
mod_files\stable-diffusion-proxy.dll
                               ...
                               転送します。バッチが
                               sdcpp_cuda\lib\stable-diffusion.dll として
                               配置します
...
※ 同梱していない (各自ダウンロード。URL は下記 2.):
   ...
   - オリジナルの stable-diffusion.dll 489MB (バッチが導入先のものを流用)
...
(3) install_sdxl.bat をダブルクリックで 1 回実行 (ゲームは閉じておく)。
    ...
    - sdcpp_cuda\lib\stable-diffusion.dll (オリジナル 489MB) をルートに
      stable-diffusion-real.dll としてコピー (プロキシの転送先。
      ルート直下でないと DLL 検索順の都合で起動時に落ちます)
    - mod_files\stable-diffusion-proxy.dll を
      sdcpp_cuda\lib\stable-diffusion.dll として配置
...
- ゲーム本体は Nuitka コンパイル済みで改変不可。そのため画像生成が呼ぶ
  sdcpp_cuda\lib\stable-diffusion.dll をプロキシに差し替えてフックしています。
...
7. アンインストール / ロールバック
--------------------------------------------------------------------------------
(1) stable-diffusion-real.dll を sdcpp_cuda\lib\stable-diffusion.dll に
    コピー (上書き) し、ルートの stable-diffusion-real.dll を削除。
